<template>
  <div class="relative flex-center wh-full" :style="{ backgroundColor: bgColor }">
    ...
    />
    <!-- <div class="absolute right-48px top-24px z-3 text-20px">
      <LanguageChange />
    </div> -->
      ...
      <div class="w-300px sm:w-360px">
        <header class="flex-y-center justify-between">
          <div class="w-70px h-70px rounded-35px overflow-hidden">
            ...
            <svg-icon local-icon="logo" style="font-size: 70px" />
          </div>
          <n-gradient-text type="primary" :size="28">{{ $t(title) }} </n-gradient-text>
        </header>
        <main class="pt-24px">
          <!-- <h3 class="text-18px text-primary font-medium">{{ $t('login.account_passwordl_ogin') }}</h3> -->
          <div>
